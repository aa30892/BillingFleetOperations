...
from sklearn.preprocessing import StandardScaler# PO vs Billing dashboard — single pre-joined file input
# Co-authored with CoCo
import streamlit as st
import pandas as pd
import numpy as np
...
def load_data(file_obj):
    ...
    if "BILLING_DT" in df.columns:
        df["BILLING_DT"] = pd.to_datetime(df["BILLING_DT"], errors="coerce")
...
def build_customer_groups(customer_names):
    ...
    for name in customer_names:
        ...
        if parts:
            prefix_map[parts[0].upper()].append(name)
    groups = {}
...
merged["CUSTOMER_GROUP"] = (
    merged["CUSTOMER_NAME_COMBINED"].map(customer_group_map).fillna(merged["CUSTOMER_NAME_COMBINED"])
)
...
# TABS
# ------------------------------------------------------------------
tab_dashboard, tab_repair, tab_anomaly = st.tabs(
    ["Dashboard", "Contract Analysis", "AI Anomaly Analysis"]
)
